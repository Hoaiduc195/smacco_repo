...
- **Components**:
  - `AIChatPanel` — Primary right control panel replacing floating widget, supports workflow cards, quick replies, progress tracking, result cards, and reference chips. Has collapse controls.
...
- **Orchestrator** (`AiOrchestratorService`): Coordinates the pipeline. For `SEARCH_PLACES`, the initial routed request yields `workflowAction: { type: 'search' }` for frontend intent confirmation and does not execute search tools. Confirmed wizard requests carry `workflowExecution.confirmed`, execute `hybrid_search -> geocode_anchor -> recommend_places`, then yield `searchAction` with final results. Compare/analyze intents still use `workflowAction` metadata for their cards.
